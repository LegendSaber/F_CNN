...
def train(epoch):
    for batch_idx, data in enumerate(train_loader, 0):
        ...
        y_pred = modeler(gray_inputs, markov_inputs)     # 前向传播
...
if __name__ == '__main__':
    ...
    test_dataset = MalwareDataset(conf.test_gray_path, conf.test_markov_path, False)
    test_loader = DataLoader(test_dataset, batch_size=conf.batch_size,
                             shuffle=False, num_workers=conf.num_workers)

    # 根据是否训练还选择是否加载保存的模型
    if conf.is_train:
        train_dataset = MalwareDataset(conf.train_gray_path, conf.train_markov_path, True)
